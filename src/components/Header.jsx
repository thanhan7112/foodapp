      ...
      <div className='hidden md:flex w-full h-full items-center justify-between'>
        ...
        <div className='flex items-center gap-8'>
          ...
          <div className='relative rounded-full'>
            <motion.img whileTap={{ scale: 0.6 }} src={user ? user.photoURL : Avatar} alt="" className='w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer rounded-full ' onClick={login} />
            {isMenu && (
              <motion.div initial={{ opacity: 0, scale: 0.6 }} exit={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0'>
                {
                  user && user.email === "thanhan7112@gmail.com" && (
                    <Link to='/createItem'>
                ...
                <p className='m-2 p-2 rounded-md shadow-md flex items-center gap-3 justify-center  bg-gray-200 cursor-pointer hover:bg-slate-300 transition-all duration-100 ease-in-out text-textColor text-base' onClick={logout}>Logout <MdLogout /></p>
              </motion.div>
            )}
          </div>
